...
const NavItem = memo(function NavItem({ to, children, highlight, onClick }) {
    ...
    if (highlight) {
        return (_jsx(NavLink, { to: to, onClick: onClick, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, className: "nav-cta", children: children }));
    }
    return (_jsx(NavLink, { to: to, onClick: onClick, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, className: ({ isActive }) => clsx('nav-link', isActive && 'nav-link--active'), "aria-current": ({ isActive }) => (isActive ? 'page' : undefined), children: children }));
});

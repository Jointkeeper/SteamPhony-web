...
describe('StarRating', () => {
    ...
    it('handles all rating values correctly', () => {
        ...
        ratings.forEach(rating => {
            const { container } = render(_jsx(StarRating, { rating: rating }));
            const filledStars = container.querySelectorAll('svg[fill="currentColor"]');
            expect(filledStars).toHaveLength(rating);
        });
    });
    it('applies custom className', () => {
        const { container } = render(_jsx(StarRating, { rating: 5, className: "custom-class" }));
        const starRating = container.querySelector('.custom-class');
        expect(starRating).toBeInTheDocument();
    });
    it('applies custom size', () => {
        const { container } = render(_jsx(StarRating, { rating: 5, size: 24 }));
    ...
    it('uses default size when not specified', () => {
        const { container } = render(_jsx(StarRating, { rating: 5 }));
        const stars = container.querySelectorAll('svg');
        stars.forEach(star => {
    ...
    it('spreads additional props to container', () => {
        const { container } = render(_jsx(StarRating, { rating: 5, "data-testid": "star-rating", title: "Rating" }));
    ...
    it('handles edge cases', () => {
        // Test minimum rating
        const { container: minContainer } = render(_jsx(StarRating, { rating: 1 }));
        const minFilledStars = minContainer.querySelectorAll('svg[fill="currentColor"]');
        expect(minFilledStars).toHaveLength(1);
        // Test maximum rating
        const { container: maxContainer } = render(_jsx(StarRating, { rating: 5 }));
    ...
    it('renders consistently with same props', () => {
        const { container: container1 } = render(_jsx(StarRating, { rating: 3 }));
        const { container: container2 } = render(_jsx(StarRating, { rating: 3 }));
